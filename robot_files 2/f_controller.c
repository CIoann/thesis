...
int msgarrvd(void *context, char *topicName, int topicLen, MQTTClient_message *message)
{
    ...
    /* UPDATE NEW INFO */
    // ==================
    vp.velocity = json_object_get_int(json_object_object_get(jobj, "velocity"));
    vp.orientation = json_object_get_int(json_object_object_get(jobj, "direction"));
    vp.locX = json_object_get_int(json_object_object_get(jobj, "location_x"));
 	vp.locY = json_object_get_int(json_object_object_get(jobj, "location_y"));
    vp.acceleration = json_object_get_int(json_object_object_get(jobj, "acceleration"));
    apply_updated_info();
...
void apply_updated_info(){
	int nspeed=10;
	printf( "Wait to receive info.. ");
	printf( "Apply info.. ");
	nspeed = max_speed*(vp.velocity+SPEED_LINEAR)/100;
	printf( "Velocity changed %d", vp.velocity );
	_run_forever(nspeed,nspeed);
}		
